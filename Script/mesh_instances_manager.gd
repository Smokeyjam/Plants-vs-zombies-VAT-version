...
var Mesh_Instance_Dict := {}  # key: plant_id (int or string), value: PackedScene or instance
...
func setupInstances():
	...
	for items in Mesh_Instance_Dict:
		var dict_num = Mesh_Instance_Dict.keys()[i]
		i += 1
		
		print("key = ",dict_num)
		var vat_multi_mesh_instance_3d = Mesh_Instance_Dict[dict_num]
